...
//------------------method---------------------
	...
	public ArrayList<PurchaseListDto> purchaseList(String USER_ID){
		...
		try {
			...
			//주문 정보 가져오는 쿼리문 
			String queryA = "select p.purchase_date, p.purchase_quantity, p.purchase_price, p.purchase_status, p.purchase_text, p.purchase_pickup_date,";
			...
			while(resultSet.next()) {
				...
				PurchaseListDto dto = new PurchaseListDto(purchase_date, cake_name, cake_img_base64Image, cake_option, purchase_text, purchase_quantity, purchase_price, purchase_status, puchase_pickup_date);
				dtos.add(dto);
			}//while() 
